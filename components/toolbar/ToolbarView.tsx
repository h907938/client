...
export const ToolbarView: FC<{ isInRoom: boolean }> = props =>
{
    ...
    const [dailyRewardsCount, setDailyRewardsCount] = useState(0); // Added state for rewards
    ...
    // Initialize daily rewards count
    useEffect(() => {
        // Check localStorage for existing count
        const storedCount = localStorage.getItem('dailyRewardsCount');
        if (storedCount) {
            setDailyRewardsCount(parseInt(storedCount));
        }
        
        // Alternatively, you could fetch from an API here
        // fetchDailyRewardsCount().then(count => setDailyRewardsCount(count));
    }, []);
        ...
        <>
            <TransitionAnimation type={ TransitionAnimationTypes.FADE_IN } inProp={ isMeExpanded } timeout={ 300 }>
                <ToolbarMeView 
                    ...
                    unseenDailyRewards={ dailyRewardsCount } 
